...
public class EffectImpl extends JdbcDaoSupport implements IEffect {
	...
	private static final String SELECT_BY_ID = "select effectId,effectName,modDT from BattleBotDb.Effect where effectId=?";
	...
	@SuppressWarnings("unchecked")
	@Override
	public EffectBean getEffectById(EffectBean effect) {
		EffectBean bean =  (EffectBean)getJdbcTemplate().queryForObject(
				SELECT_BY_ID, new Object[]{effect.getEffectId()}, new EffectRowMapper());
		return bean;
	}
	...
	@Override
	public boolean effectIsPresent(EffectBean effect) {
		try{
			 getEffectById(effect);
			 return true;
		}catch(Exception e){
			return false;
		}
